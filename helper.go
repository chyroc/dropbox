...
func httpRequest(method, url string, body io.Reader, headers map[string]string, res interface{}) (*http.Response, []byte, *Error) {
	...
	if err != nil {
		return nil, nil, NewError(ErrRequestFail, fmt.Sprintf("%s; method=%s, url=%s", err, method, url))
	}
	...
	if err != nil {
		return nil, nil, NewError(ErrRequestFail, err.Error())
	}
	...
	if res != nil {
		if err = json.Unmarshal(bs, res); err != nil {
			return nil, bs, NewError(ErrRequestFail, err.Error())
		}
	}
